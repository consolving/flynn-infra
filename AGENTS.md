...
- **No uncommitted patches**: Never leave working changes uncommitted in submodules. If there are unstaged modifications in `flynn/` or `flynn-tuf-repo/`, they should be committed (with proper subsystem prefix) and pushed before finishing work. Check with `git submodule status` — a `+` prefix indicates the submodule HEAD differs from the recorded commit, and `modified content` in `git status` indicates uncommitted changes inside the submodule.
