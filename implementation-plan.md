...
- Proper **source-side** fix for the layer-sync gap (write ALL layers flat into `repository/targets/` every run in `script/export-tuf/main.go`) so future releases cannot ship with missing layers on IPFS — **still open** (the mirror-side layer-store now guards against loss, but `export-tuf` itself still doesn't write all changed layers flat; it relies on `layer-cache` + "assume IPFS already has it").
