...
- **Router never re-resolves controller service**: the router held the controller event-stream IP (resolved once at startup) forever; after a controller rollout the old web job IP (`100.100.49.4`) was dead and every router logged `sync error: i/o timeout` / `no route to host` indefinitely. Workaround: `flynn -a router kill <job>` (scheduler respawns, fresh DNS lookup). **Fixed 2026-09-12** (flynn branch `feat/router-controller-failover`, commit `39d4247f`, pending cluster validation): the route/cert sync now watches the controller service's discoverd instance events and fails the stream when the pinned instance goes away, so the listener re-resolves a live controller and re-syncs from scratch; per-instance connect timeouts bound failover latency, and `pkg/httpclient.ResumingStream` now honors `Close()` during its resume phase (previously a stopped stream kept reconnecting to the dead address in the background). Regression test: `router/store_test.go` `TestControllerStoreFailover` (real discoverd + fake controller instances).
...
- [x] Fix router controller-stream re-resolution on failure — 2026-09-12, flynn branch `feat/router-controller-failover` (commit `39d4247f`): discoverd-instance-watch failover + stop-aware resuming stream; unit-tested (`TestControllerStoreFailover`), pending cluster validation.
...
- [x] Open a PR for `feat/letsencrypt` referencing issue #12 — done: flynn PR #14, merged to main 2026-09-12 (`c358b7b0`).
...
- **Follow-up (fixed 2026-09-12)**: this was a latent architecture gap, not specific to today's incidents — *any* time every controller instance the external router was originally watching got replaced, route/cert sync silently went stale until the router itself was restarted, with no self-healing and no obvious error surfaced anywhere. Fixed in flynn `feat/router-controller-failover` (`39d4247f`): `router/store.go`'s `Watch` now subscribes to the controller service's discoverd instance-change events and, when the pinned instance goes down, fails the event stream so the sync re-resolves a live controller and re-syncs from scratch (no lost events: the initial route list is re-fetched).
